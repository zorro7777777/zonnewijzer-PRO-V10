// SunCalc 1.9.0 inline (c) Vladimir Agafonkin, BSD-2-Clause
(function(){
'use strict';
...
SunCalc.getPosition=function(date,lat,lng){
  ...
  return{azimuth:atan(sin(H),cos(H)*sin(phi)-tan(c.dec)*cos(phi)),altitude:asin(sin(phi)*sin(c.dec)+cos(phi)*cos(c.dec)*cos(H))};
};
...
SunCalc.getTimes=function(date,lat,lng,height){
  height=height||0;
  var lw=rad*-lng,phi=rad*lat,dh=observerAngle(height),d=toDays(date),n=julianCycle(d,lw),
      ds=approxTransit(0,lw,n),M=solarMeanAnomaly(ds),L=eclipticLongitude(M),
      dec=declination(L,0),Jnoon=solarTransitJ(ds,M,L),
      result={solarNoon:fromJulian(Jnoon),nadir:fromJulian(Jnoon-0.5)},i,len,time,h0,Jset,Jrise;
  for(i=0,len=times.length;i<len;i++){
    time=times[i];h0=(time[0]+dh)*rad;
    Jset=getSetJ(h0,lw,phi,dec,n,M,L);Jrise=Jnoon-(Jset-Jnoon);
...
SunCalc.getMoonPosition=function(date,lat,lng){
  ...
  return{azimuth:atan(sin(H),cos(H)*sin(phi)-tan(c.dec)*cos(phi)),altitude:h,distance:c.dist};
};
